...
export function ActionForm({ action, onSuccess, children, ...props }: ActionFormProps) {
  ...
  return (
    <>
      ...
      {showMessage && mounted && document.body && createPortal(
        <div className="fixed top-6 right-6 z-[100] flex flex-col gap-2 pointer-events-none">
          {state.success && (
            <div className="pointer-events-auto flex w-80 items-start gap-3 rounded-lg border border-green-200 bg-green-50 p-4 shadow-lg animate-in slide-in-from-top-5 fade-in duration-300 dark:border-green-400/40 dark:bg-green-500/10">
              ...
              <button type="button" onClick={() => setShowMessage(false)} className="text-green-500 transition-colors hover:text-green-700 dark:text-green-300 dark:hover:text-green-100">
                <X className="h-4 w-4" />
              </button>
            </div>
          )}
          ...
          {state.error && (
            <div className="pointer-events-auto flex w-80 items-start gap-3 rounded-lg border border-red-200 bg-red-50 p-4 shadow-lg animate-in slide-in-from-top-5 fade-in duration-300 dark:border-red-400/40 dark:bg-red-500/10">
              ...
              <button type="button" onClick={() => setShowMessage(false)} className="text-red-500 transition-colors hover:text-red-700 dark:text-red-300 dark:hover:text-red-100">
                <X className="h-4 w-4" />
              </button>
            </div>
          )}
